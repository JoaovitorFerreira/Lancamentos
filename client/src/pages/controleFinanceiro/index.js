...
import LinhaLancamento from "./components/LineEntry";
import Filtro from "./components/Filter";
import Spinner from "../../components/Spinner";
import api from "../../services/api";
import { Container, Content, Title, Title2, Acoes, Lista } from "./styles";
import SeletorMeses from "./components/MonthSelector";
import Totalizador from "./components/Total";
...
export default function ControleFinaceiro(props) {
  ...
  const [currentMes, setCurrentMes] = useState(
    (props.match.params.period
      ? moment(props.match.params.period)
      : moment()
    ).format("YYYY-MM")
  );
